<?php
	...
	$id=$_GET["id"];
	$ui_url=attachToCurrentURLInPath("NAndUI.php");
	$ui_factory_url=attachToCurrentURLInPath("NAndFactoryUI.php");
?>
<script>
	
	function NAndComponent(id, type){
		var objSelf = this;
		this.id=id;
	...
	function NAndComponentFactory(type){
		var objSelf = this;
		this.ui_factory_url="<?php echo $ui_factory_url?>";
		this.type=type;
		this.uicomponent=null;
		this.createComponent=function(id){
			component=new NAndComponent(id,objSelf.type);
			return component;
			};
	...
	workspace.addComponentFactory(new NAndComponentFactory("<?php echo $id ?>"));
</script>
