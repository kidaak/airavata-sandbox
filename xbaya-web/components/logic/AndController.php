<?php
	...
	$id=$_GET["id"];
	$ui_url=attachToCurrentURLInPath("AndUI.php");
	$ui_factory_url=attachToCurrentURLInPath("AndFactoryUI.php");
?>
<script>
	
	function AndComponent(id, type){
		var objSelf = this;
		this.id=id;
	...
	function AndComponentFactory(type){
		var objSelf = this;
		this.ui_factory_url="<?php echo $ui_factory_url?>";
		this.type=type;
		this.uicomponent=null;
		this.createComponent=function(id){
			component=new AndComponent(id,objSelf.type);
			return component;
			};
	...
	workspace.addComponentFactory(new AndComponentFactory("<?php echo $id ?>"));
</script>
